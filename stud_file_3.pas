...
{---процедура чтения из файла слов и записи их в 12-литерные строки (с пробелами справа)---}
...
begin
    if List<>nil then begin
        delete(List^.data.Surname,(pos(' ',List^.data.Surname)+1),(12-pos(' ',List^.data.Surname)));
        delete(List^.data.Name,(pos(' ',List^.data.Name)+1),(12-pos(' ',List^.data.Name)));
        write(concat(List^.data.Surname,List^.data.Name,List^.data.Otch,'|'):36);
        write(List^.data.Group:9,'|',List^.data.Birth.year:14,'|');
        delete(List^.data.City,pos(' ',List^.data.City),(12-pos(' ',List^.data.City)));
        writeln(List^.data.City:18,'|');
        printList(List^.next)
    end
end;
...
begin
    tempList:=List;
    Writeln('Значения заданных параметров:');
    writeln('------------------------------------------------');
    writeln('Параметр - Фамилия');
    writeln('------------------------------------------------');
    While tempList<>nil do begin
        if tempList^.tag='Surname' then begin
            writeln(tempList^.value,' встречается - ',tempList^.ModaCount,' раз(а)')
        end;
        templist:=tempList^.next
    end;
    writeln('------------------------------------------------');
    tempList:=List;
    writeln('Параметр - Имя');
    writeln('------------------------------------------------');
    While tempList<>nil do begin
    ...
    writeln('------------------------------------------------');
    tempList:=List;
    writeln('Параметр - Год рождения');
    writeln('------------------------------------------------');
    While tempList<>nil do begin
        if tempList^.tag='Year' then begin
    writeln(tempList^.value,' встречается - ':36,tempList^.ModaCount,' раз(а)');
        end;
        templist:=tempList^.next
    end;
    writeln('------------------------------------------------');
...
{---тело программы---}
begin
    assign(inputFile,'students_3rd_year.txt');
    ...
    Writeln('ФИО |':39,'№ группы |':6,' Год рождения |':12,'Город|':24);
    writeln('--------------------------------------------------------------------------------');
    printList(shortList);
